...
const beginnerGoal = (x: number, y: number, label: string, requiredHeading?: number) => goal(x, y, label, 0, requiredHeading);
...
function makeLayout(level: ArenaLevel, number: number, title: string, objective: string, hint: string, mainPath: ArenaPoint[], options: LayoutOptions = {}): OBRLayout {
  ...
  return {
    ...
    floorMarkers: options.floorMarkers ?? [], greenMarkers: options.greenMarkers ?? [], hazards,
